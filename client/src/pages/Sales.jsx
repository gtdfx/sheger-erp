  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        <button onClick={() => setTab('apartments')} className={`px-4 py-2 rounded-sm text-sm font-semibold transition-colors ${tab === 'apartments' ? 'bg-[rgba(30,77,140,0.12)] text-[#1e4d8c] border border-[#1e4d8c]/20' : 'text-[#141b2d]-muted hover:text-[#141b2d] hover:bg-[#f4f6fb]'}`}>
          <Home className="w-4 h-4 inline mr-1.5" /> Apartments ({apartments.length})
        </button>
        <button onClick={() => setTab('sales')} className={`px-4 py-2 rounded-sm text-sm font-semibold transition-colors ${tab === 'sales' ? 'bg-[rgba(30,77,140,0.12)] text-[#1e4d8c] border border-[#1e4d8c]/20' : 'text-[#141b2d]-muted hover:text-[#141b2d] hover:bg-[#f4f6fb]'}`}>
      ...
      {tab === 'apartments' && (
        <div className="space-y-4">
          <div className="flex justify-end"><button onClick={() => setShowAddApt(!showAddApt)} className="btn-primary"><Plus className="w-4 h-4" /> Add Apartment</button></div>
          {showAddApt && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {apartments.map(apt => (
              <div key={apt.id} className="card-hover p-5">
              ...
              {sales.map(sale => (
                <div key={sale.id} className="card p-5">
                  <div className="flex items-center gap-4">
                    <button onClick={() => setExpandedSale(e => ({...e, [sale.id]: !e[sale.id]}))} className="text-[#141b2d]-muted">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 mt-1 text-sm text-[#141b2d]-secondary">
                        <span className="flex items-center gap-1"><User className="w-3.5 h-3.5" />{sale.buyer_name}</span>
                        {sale.buyer_phone && <span className="flex items-center gap-1"><Phone className="w-3.5 h-3.5" />{sale.buyer_phone}</span>}
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="font-display font-bold text-[#1e4d8c]">{fmt(sale.total_price)} ETB</p>
